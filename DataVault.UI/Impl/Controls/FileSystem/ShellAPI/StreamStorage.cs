...
namespace DataVault.UI.Impl.Controls.FileSystem.ShellAPI
{
    internal class StreamStorageProvider : IFileInfoProvider, IDirInfoProvider
    {
        ...
        private NativeShellAPI.STATSTG dirInfo;
        private bool dirInfoRetrieved = false;

        public NativeShellAPI.STATSTG GetDirInfo()
        {
            if (dirInfoRetrieved)
                return dirInfo;
            else if (providerItem != null)
            {
                ...
                {
                    tempStorage.Stat(out dirInfo, NativeShellAPI.STATFLAG.NONAME);
    ...
    public class ShellStreamReader : Stream, IDisposable
    {
        ...
        private NativeShellAPI.STATSTG streamInfo;
        ...
        internal NativeShellAPI.STATSTG StreamInfo
        {
            get
            {
                if (!streamInfoRead)
                {
                    stream.Stat(out streamInfo, NativeShellAPI.STATFLAG.NONAME);
                    streamInfoRead = true;
                }
                return streamInfo;
            }
        ...
        {
            NativeShellAPI.STGM grfmode = NativeShellAPI.STGM.SHARE_DENY_WRITE;
            
            switch (access)
            {
                case FileAccess.ReadWrite:
                    grfmode |= NativeShellAPI.STGM.READWRITE;
                    break;

                case FileAccess.Write:
                    grfmode |= NativeShellAPI.STGM.WRITE;
                    break;
            }
            ...
            if (parentStorage != null)
            {
                ...
                else if (access != FileAccess.Read)
                {
                    if (parentStorage.OpenStream(
                        shellItem.Text + (shellItem.IsLink ? ".lnk" : string.Empty),
                        IntPtr.Zero,
                        NativeShellAPI.STGM.SHARE_DENY_WRITE,
                        0,
                        out streamPtr) == NativeShellAPI.S_OK)
                    {
