// This code is copy-pasted from the CodeProject's article by Steven Roebert:
// http://www.codeproject.com/KB/miscctrl/FileBrowser.aspx
//
// Aforementioned article "has no explicit license attached to it", 
// neither has a code download at http://www.codeproject.com/KB/miscctrl/FileBrowser/FileBrowser_src.zip
// so I felt free to reuse the source code for my application.
// 
// In the case of any licensing troubles, I'm ready to remove this code from DataVault's codebase.
...
using DataVault.UI.Impl.Controls.FileSystem.Controls;
using DataVault.UI.Impl.Controls.FileSystem.ShellAPI;
using DataVault.UI.Impl.Controls.FileSystem.ShellAPI.Interfaces;
using System.IO;
using System.Linq;

namespace DataVault.UI.Impl.Controls.FileSystem
{
    #region Delegates

    // This delegate is used for the ContextMenuHoverEvent
    internal delegate void ContextMenuMouseHoverEventHandler(object sender, ContextMenuMouseHoverEventArgs e);

    // This delegate is used for the SelectedFolderChangedEvent
    internal delegate void SelectedFolderChangedEventHandler(object sender, SelectedFolderChangedEventArgs e);
    ...
    /// </summary>
    internal partial class ShellBrowser : UserControl
    {
        #region Fields

        // The shellbrowser used by this browser to get the shellitems of all files and folders
        private ShellBrowserComponent shellBrowserComponent;
        ...
        private ShellItem currentDirectory;
        ...
        private void InitUpdate()
        {
            ...
            ShellBrowserComponent.ShellItemUpdate += new ShellItemUpdateEventHandler(shellBrowser_ShellItemUpdate);
            updateThread.Start();
        }
        ...
        void FileBrowser_HandleDestroyed(object sender, EventArgs e)
        {
            if (handleCreated)
            {
                ...
                this.updating = false;
                this.ShellBrowserComponent.UpdateCondition.ContinueUpdate = false;

                // not sure whether this is necessary. requires additional investigation
//                this.updateThread.IsBackground = false;
        ...
        void folderView_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (selectionChange)
            {
                ShellItem oldItem = currentDirectory;
        ...
        private void UpdateLoop()
        {
            while (updating)
            {
                ...
                {
                    ShellBrowserComponent.UpdateCondition.ContinueUpdate = true;
                    currentDirectory.Update(true, true);
                }
        ...
        private string ConvertPath(string path)
        {
            ...
            if (newPath.StartsWith(
                    string.Format(@"{0}\", ShellBrowserComponent.MyComputerName),
                    false,
        ...
        public TreeNode SelectPath(SpecialFolders specialFolder, bool expandNode)
        {
            ...
            else if (NativeShellAPI.SHGetFolderPath(
                    IntPtr.Zero, (NativeShellAPI.CSIDL)specialFolder,
                    IntPtr.Zero, NativeShellAPI.SHGFP.TYPE_CURRENT, path) == NativeShellAPI.S_OK)
            {
                path.Replace(ShellBrowserComponent.MyDocumentsPath, ShellBrowserComponent.MyDocumentsName);
            ...
            else
            {
                ...
                if (specialFolder == SpecialFolders.MyDocuments)
                {
                    ...
                    ShellBrowserComponent.DesktopItem.ShellFolder.ParseDisplayName(
                        IntPtr.Zero,
                        IntPtr.Zero,
                ...
                {
                    NativeShellAPI.SHGetSpecialFolderLocation(
                        IntPtr.Zero,
                        (NativeShellAPI.CSIDL)specialFolder,
                        out pidl);
                }
                ...
                if (pidl != IntPtr.Zero)
                {
                    ...
                    if (ShellBrowserComponent.DesktopItem.ShellFolder.GetDisplayNameOf(
                                    pidl,
                                    NativeShellAPI.SHGNO.FORADDRESSBAR | NativeShellAPI.SHGNO.FORPARSING,
                                    strr) == NativeShellAPI.S_OK)
                    {
                        NativeShellAPI.StrRetToBuf(strr, pidl, buf, NativeShellAPI.MAX_PATH);
                    }
        ...
        public TreeNode SelectPath(ShellItem item, bool expandNode)
        {
            ...
            ShellItem[] path = ShellBrowserComponent.GetPath(item);
        ...
        {
            get { return fileView.MultiSelect; }
            set { fileView.MultiSelect = value; }
        }

        [Browsable(false)]
        public TreeNode CurrentDirectoryNode
        {
        ...
        [Browsable(false)]
        public ShellItem CurrentDirectory
        {
        ...
        public ShellItem[] SelectedItems
        {
            get
            {
                return fileView.SelectedOrder.Cast<ListViewItem>().Select(lvi => (ShellItem)lvi.Tag).ToArray();
            }
        }
        ...
        [Category("Options"),
         ...
         DefaultValue(null),
         Browsable(true)]
        public ShellBrowserComponent ShellBrowserComponent
        {
            get { return shellBrowserComponent; }
            set
            {
                if (!ShellBrowserComponent.Equals(ShellBrowserComponent, value))
                {
                    if (ShellBrowserComponent != null)
                        ShellBrowserComponent.Browsers.Remove(this);

                    if (handleCreated) FileBrowser_HandleDestroyed(this, new EventArgs());
                    shellBrowserComponent = value;
                    if (handleCreated) FileBrowser_HandleCreated(this, new EventArgs());

                    if (!ShellBrowserComponent.Browsers.Contains(this))
                        ShellBrowserComponent.Browsers.Add(this);
                }
            }
        }
        ...
        {
            if (currentDirectory.IsFileSystem)
            {
                IntPtr newMenuPtr;
                IContextMenu newMenu;

                if (ContextMenuHelper.GetNewContextMenu(currentDirectory, out newMenuPtr, out newMenu))
                {
                    lock (ShellBrowserComponent)
                    {
                        NewItemCreated = true;
                    }

                    ContextMenuHelper.InvokeCommand(newMenu, "NewFolder", ShellItem.GetRealPath(currentDirectory), new Point(0, 0));
        ...
        {
            if (Control.Equals(dragStartControl, e.DragStartControl) && (e.MouseButtons & NativeShellAPI.MK.RBUTTON) != 0)
            {
        ...
        void navAddressBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (navAddressBox.SelectedIndex > -1)
            {
                ...
                if (!currentDirectory.Equals(item))
                {
                    TreeNode oldNode = currentDirectoryNode;
                    int currentIndex = navAddressBox.Items.IndexOf(navAddressBox.CurrentItem);
                    BrowserComboItem currentItem = navAddressBox.CurrentItem;
                    int maxIndent = folderView.IsParentNode(myCompNode, currentDirectoryNode) ? 3 : 2;
        ...
        private void navBackForwardButton_ButtonClick(object sender, EventArgs e)
        {
            ...
            {
                AddNavBackForwardItem(navForwardButton, currentDirectory);
            }
            else
            {
                AddNavBackForwardItem(navBackButton, currentDirectory);
            }
    ...
    internal class ContextMenuMouseHoverEventArgs : EventArgs
    {
        private string info;
    ...
    internal class SelectedFolderChangedEventArgs : EventArgs
    {        
