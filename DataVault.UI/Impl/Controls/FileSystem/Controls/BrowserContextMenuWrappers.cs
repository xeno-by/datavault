...
namespace DataVault.UI.Impl.Controls.FileSystem.Controls
{
    ...
    internal class BrowserTVContextMenuWrapper : NativeWindow
    {
        ...
        {
            ExpandCollapse = (int)NativeShellAPI.CMD_LAST + 1
        }
        ...
        public BrowserTVContextMenuWrapper(ShellBrowser br)
        {
            this.br = br;
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (iContextMenu != null &&
                m.Msg == (int)NativeShellAPI.WM.MENUSELECT &&
                ((int)ShellHelper.HiWord(m.WParam) & (int)NativeShellAPI.MFT.SEPARATOR) == 0 &&
                ((int)ShellHelper.HiWord(m.WParam) & (int)NativeShellAPI.MFT.POPUP) == 0)
            {
                ...
                else
                {
                    info = ContextMenuHelper.GetCommandString(
                        iContextMenu,
                        ShellHelper.LoWord(m.WParam) - NativeShellAPI.CMD_FIRST,
                        false);
                }
            ...
            if (iContextMenu2 != null &&
                (m.Msg == (int)NativeShellAPI.WM.INITMENUPOPUP ||
                 m.Msg == (int)NativeShellAPI.WM.MEASUREITEM ||
                 m.Msg == (int)NativeShellAPI.WM.DRAWITEM))
            {
                if (iContextMenu2.HandleMenuMsg(
                    (uint)m.Msg, m.WParam, m.LParam) == NativeShellAPI.S_OK)
                    return;
            }
            ...
            if (iContextMenu3 != null &&
                m.Msg == (int)NativeShellAPI.WM.MENUCHAR)
            {
                if (iContextMenu3.HandleMenuMsg2(
                    (uint)m.Msg, m.WParam, m.LParam, IntPtr.Zero) == NativeShellAPI.S_OK)
                    return;
            }
        ...
        void FolderView_BeforeLabelEdit(object sender, NodeLabelEditEventArgs e)
        {
            ...
            if (item.IsDisk)
            {
                IntPtr editHandle = NativeShellAPI.SendMessage(br.FolderView.Handle, NativeShellAPI.WM.TVM_GETEDITCONTROL, 0, IntPtr.Zero);
                NativeShellAPI.SendMessage(editHandle, NativeShellAPI.WM.SETTEXT, 0,
        ...
        void FolderView_AfterLabelEdit(object sender, NodeLabelEditEventArgs e)
        {
            ...
            if (e.Label != null && !(item.IsDisk && item.Text.Substring(0, item.Text.LastIndexOf(' ')) == e.Label) &&
                item.ParentItem.ShellFolder.SetNameOf(
                    ...
                    item.PIDLRel.Ptr,
                    e.Label,
                    NativeShellAPI.SHGNO.NORMAL,
                    out newPidl) == NativeShellAPI.S_OK)
            {
        ...
        void FolderView_MouseUp(object sender, MouseEventArgs e)
        {
                ...
                hitTest.Location == TreeViewHitTestLocations.StateImage))
            {
                ...
                try
                {
                    ...
                    {
                        contextMenu = NativeShellAPI.CreatePopupMenu();

                        iContextMenu.QueryContextMenu(
                            contextMenu,
                            0,
                            NativeShellAPI.CMD_FIRST,
                            NativeShellAPI.CMD_LAST,
                            NativeShellAPI.CMF.EXPLORE |
                            NativeShellAPI.CMF.CANRENAME |
                            ((Control.ModifierKeys & Keys.Shift) != 0 ? NativeShellAPI.CMF.EXTENDEDVERBS : 0));

                        string topInvoke = hitTest.Node.IsExpanded ? "Collapse" : "Expand";
                        NativeShellAPI.MFT extraFlag = (hitTest.Node.Nodes.Count > 0) ? 0 : NativeShellAPI.MFT.GRAYED;
                        NativeShellAPI.InsertMenu(contextMenu, 0,
                            NativeShellAPI.MFT.BYPOSITION | extraFlag,
                            (int)CMD_CUSTOM.ExpandCollapse, topInvoke);
                        NativeShellAPI.InsertMenu(contextMenu, 1,
                            NativeShellAPI.MFT.BYPOSITION | NativeShellAPI.MFT.SEPARATOR,
                            0, "-");

                        NativeShellAPI.SetMenuDefaultItem(
                            contextMenu,
                            0,
                            true);

                        Marshal.QueryInterface(iContextMenuPtr, ref NativeShellAPI.IID_IContextMenu2, out iContextMenuPtr2);
                        Marshal.QueryInterface(iContextMenuPtr, ref NativeShellAPI.IID_IContextMenu3, out iContextMenuPtr3);
                        ...
                        uint selected = NativeShellAPI.TrackPopupMenuEx(
                                            contextMenu,
                                            NativeShellAPI.TPM.RETURNCMD,
                                            ptInvoke.X,
                                            ptInvoke.Y,
                                            this.Handle,
                                            IntPtr.Zero);
                        ...
                        else if (selected >= NativeShellAPI.CMD_FIRST)
                        {
                            string command = ContextMenuHelper.GetCommandString(
                                iContextMenu,
                                selected - NativeShellAPI.CMD_FIRST,
                                true);
                ...
                finally
                {
                    ...
                    if (contextMenu != null)
                        NativeShellAPI.DestroyMenu(contextMenu);
    ...
    internal class BrowserLVContextMenuWrapper : NativeWindow
    {
        ...
        private enum CMD_CUSTOM
        {
            Tiles = (int)NativeShellAPI.CMD_LAST + 1,
            Icons,
            List,
            Details,
            Properties,
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (newContextMenu2 != null &&
                ((m.Msg == (int)NativeShellAPI.WM.INITMENUPOPUP && m.WParam == newSubmenuPtr) ||
                 m.Msg == (int)NativeShellAPI.WM.MEASUREITEM ||
                 m.Msg == (int)NativeShellAPI.WM.DRAWITEM))
            {
        ...
        void FileView_BeforeLabelEdit(object sender, LabelEditEventArgs e)
        {
            ...
            {
                IntPtr editHandle = NativeShellAPI.SendMessage(br.FileView.Handle, NativeShellAPI.WM.LVM_GETEDITCONTROL, 0, IntPtr.Zero);
                NativeShellAPI.SendMessage(editHandle, NativeShellAPI.WM.SETTEXT, 0,
                    Marshal.StringToHGlobalAuto(item.Text.Substring(0, item.Text.LastIndexOf(' '))));
            }
        }
                        ...
                        try
                        {
                            if (ContextMenuHelper.GetIContextMenu(parentShellFolder, pidls, out icontextMenuPtr, out iContextMenu))
                            {
                                iContextMenu.QueryContextMenu(
                                    ...
                                    NativeShellAPI.CMD_FIRST,
                                    NativeShellAPI.CMD_LAST,
                                    NativeShellAPI.CMF.DEFAULTONLY);

                                int defaultCommand = NativeShellAPI.GetMenuDefaultItem(contextMenu.Handle, false, 0);
                                if (defaultCommand >= NativeShellAPI.CMD_FIRST)
                                {
                                    ContextMenuHelper.InvokeCommand(
                                        iContextMenu,
                                        (uint)defaultCommand - NativeShellAPI.CMD_FIRST,
        ...
        private void CreateNormalMenu(Point ptInvoke, ListViewHitTestInfo hitTest, MouseEventArgs e)
        {
            ...
            try
            {
                if (ContextMenuHelper.GetIContextMenu(parentShellFolder, pidls, out iContextMenuPtr, out iContextMenu))
                {
                    ...
                    if (selected >= NativeShellAPI.CMD_FIRST)
                    {
                        string command = ContextMenuHelper.GetCommandString(iContextMenu, selected - NativeShellAPI.CMD_FIRST, true);
                        ...
                        else
                        {
                            ContextMenuHelper.InvokeCommand(
                                iContextMenu,
                                selected - NativeShellAPI.CMD_FIRST,
                                ShellItem.GetRealPath(br.CurrentDirectory),
                                ptInvoke);
                        }
                    }
                }
        ...
        private void CreateFolderMenu(Point ptInvoke, MouseEventArgs e)
        {
            ...
            ShellItem currentItem = br.CurrentDirectory;
            ...
            try
            {
                ...
                contextMenu = NativeShellAPI.CreatePopupMenu();
                viewSubMenu = NativeShellAPI.CreatePopupMenu();

                #region View Submenu

                NativeShellAPI.MENUITEMINFO itemInfo = new NativeShellAPI.MENUITEMINFO("View");
                itemInfo.cbSize = NativeShellAPI.cbMenuItemInfo;
                itemInfo.fMask = NativeShellAPI.MIIM.SUBMENU | NativeShellAPI.MIIM.STRING;
                itemInfo.hSubMenu = viewSubMenu;
                NativeShellAPI.InsertMenuItem(contextMenu, 0, true, ref itemInfo);

                NativeShellAPI.MFT rCheck = NativeShellAPI.MFT.RADIOCHECK | NativeShellAPI.MFT.CHECKED;
                NativeShellAPI.AppendMenu(viewSubMenu,
                ...
                NativeShellAPI.AppendMenu(contextMenu, NativeShellAPI.MFT.SEPARATOR, 0, string.Empty);
                NativeShellAPI.AppendMenu(contextMenu, canPaste ? 0 : NativeShellAPI.MFT.GRAYED, (int)CMD_CUSTOM.Paste, "Paste");
                NativeShellAPI.AppendMenu(contextMenu, canPasteShortCut ? 0 : NativeShellAPI.MFT.GRAYED, (int)CMD_CUSTOM.Paste_ShortCut, "Paste Shortcut");

                #endregion

                #region New Submenu

                if (br.CurrentDirectory.IsFileSystem &&
                    ContextMenuHelper.GetNewContextMenu(br.CurrentDirectory, out newContextMenuPtr, out newContextMenu))
                {
                    NativeShellAPI.AppendMenu(contextMenu, NativeShellAPI.MFT.SEPARATOR, 0, string.Empty);
                    newContextMenu.QueryContextMenu(
                        ...
                        NativeShellAPI.CMD_FIRST,
                        NativeShellAPI.CMD_LAST,
                        NativeShellAPI.CMF.NORMAL);

                    newSubmenuPtr = NativeShellAPI.GetSubMenu(contextMenu, 5);

                    Marshal.QueryInterface(newContextMenuPtr, ref NativeShellAPI.IID_IContextMenu2, out newContextMenuPtr2);
                    Marshal.QueryInterface(newContextMenuPtr, ref NativeShellAPI.IID_IContextMenu3, out newContextMenuPtr3);
                ...
                if (!br.CurrentDirectory.Equals(br.ShellBrowserComponent.DesktopItem))
                {
                    NativeShellAPI.AppendMenu(contextMenu, NativeShellAPI.MFT.SEPARATOR, 0, string.Empty);
                    NativeShellAPI.AppendMenu(contextMenu, 0, (int)CMD_CUSTOM.Properties, "Properties");
                }
                ...
                CMD_CUSTOM selected = (CMD_CUSTOM)NativeShellAPI.TrackPopupMenuEx(
                                    contextMenu,
                                    NativeShellAPI.TPM.RETURNCMD,
                                    ptInvoke.X,
                                    ptInvoke.Y,
                ...
                if ((int)selected >= NativeShellAPI.CMD_FIRST)
                {
                    switch (selected)
                    {
                        #region Normal Views
                        ...
                        case CMD_CUSTOM.Properties:
                            ContextMenuHelper.InvokeCommand(
                                br.CurrentDirectory.ParentItem,
                                new IntPtr[] { br.CurrentDirectory.PIDLRel.Ptr },
                                "properties",
                                ptInvoke);
                            break;
                        ...
                        default:
                            #region New
                            if ((uint)selected <= NativeShellAPI.CMD_LAST)
                            {
                                lock (br.ShellBrowserComponent)
                                {
                                ...
                                ContextMenuHelper.InvokeCommand(
                                    newContextMenu,
                                    (uint)selected - NativeShellAPI.CMD_FIRST,
                                    ShellItem.GetRealPath(br.CurrentDirectory),
                                    ptInvoke);
                            }
                            #endregion
            ...
            finally
            {
                ...
                if (viewSubMenu != null)
                    NativeShellAPI.DestroyMenu(viewSubMenu);
        ...
        void SetToolTip(ListViewItem listItem)
        {
            ...
            {
                string info;
                queryInfo.GetInfoTip(NativeShellAPI.QITIPF.DEFAULT, out info);

                //if (string.IsNullOrEmpty(info))
                //queryInfo.GetInfoTip(NativeShellAPI.QITIPF.USESLOWTIP, out info);
    ...
    internal static class ContextMenuHelper
    {
        ...
        public static string GetCommandStringA(IContextMenu iContextMenu, uint idcmd, bool executeString)
        {
            ...
            iContextMenu.GetCommandString(
                idcmd,
                (executeString ? NativeShellAPI.GCS.VERBA : NativeShellAPI.GCS.HELPTEXTA),
                0,
                bytes,
                NativeShellAPI.MAX_PATH);
        ...
        public static string GetCommandStringW(IContextMenu iContextMenu, uint idcmd, bool executeString)
        {
            ...
            iContextMenu.GetCommandString(
                idcmd,
                (executeString ? NativeShellAPI.GCS.VERBW : NativeShellAPI.GCS.HELPTEXTW),
                0,
                bytes,
                NativeShellAPI.MAX_PATH);
            ...
            out IContextMenu iContextMenu)
        {
            if (parent.GetUIObjectOf(
                        ...
                        (uint)pidls.Length,
                        pidls,
                        ref NativeShellAPI.IID_IContextMenu,
                        IntPtr.Zero,
                        out iContextMenuPtr) == NativeShellAPI.S_OK)
            {
                iContextMenu =
        ...
        public static bool GetNewContextMenu(ShellItem item, out IntPtr iContextMenuPtr, out IContextMenu iContextMenu)
        {
            if (NativeShellAPI.CoCreateInstance(
                    ref NativeShellAPI.CLSID_NewMenu,
                    IntPtr.Zero,
                    ...
                    out iContextMenuPtr) == NativeShellAPI.S_OK)
            {
                ...
                if (Marshal.QueryInterface(
                    iContextMenuPtr,
                    ref NativeShellAPI.IID_IShellExtInit,
                    out iShellExtInitPtr) == NativeShellAPI.S_OK)
                {
        ...
        /// Ctrl + C and Ctrl + V.
        /// </summary>
        public static void ProcessKeyCommands(ShellBrowser br, object sender, KeyEventArgs e)
        {
                    ...
                    case Keys.X:
                        #region Copy/Paste/Cut
                        {
                            ...
                            if (sender.Equals(br.FileView) && e.KeyCode != Keys.V)
                            {
                                ...
                                parent = br.CurrentDirectory;
                            }
                            else
                            {
                                pidls = new IntPtr[1];
                                pidls[0] = br.CurrentDirectory.PIDLRel.Ptr;
                                parent = (br.CurrentDirectory.ParentItem != null ? br.CurrentDirectory.ParentItem : br.CurrentDirectory);
                            }
                ...
                switch (e.KeyCode)
                {
                    case Keys.Insert:
                        ...
                        if (e.Shift && !e.Control && !e.Alt)
                        {
                            ...
                            pidls[0] = br.CurrentDirectory.PIDLRel.Ptr;
                            ShellItem parent = (br.CurrentDirectory.ParentItem != null ? br.CurrentDirectory.ParentItem : br.CurrentDirectory);
