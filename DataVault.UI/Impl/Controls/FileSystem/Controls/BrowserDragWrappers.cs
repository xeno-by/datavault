...
namespace DataVault.UI.Impl.Controls.FileSystem.Controls
{
    ...
    internal class BrowserTVDragWrapper : IDropSource, IDisposable
    {
        ...
        public BrowserTVDragWrapper(ShellBrowser br)
        {
            this.br = br;
        ...
        public int QueryContinueDrag(bool fEscapePressed, NativeShellAPI.MK grfKeyState)
        {
            if (fEscapePressed)
                return NativeShellAPI.DRAGDROP_S_CANCEL;
            else
            {
                if ((startButton & MouseButtons.Left) != 0 && (grfKeyState & NativeShellAPI.MK.LBUTTON) == 0)
                    return NativeShellAPI.DRAGDROP_S_DROP;
                else if ((startButton & MouseButtons.Right) != 0 && (grfKeyState & NativeShellAPI.MK.RBUTTON) == 0)
                    return NativeShellAPI.DRAGDROP_S_DROP;
                else
        ...
        {
            return NativeShellAPI.DRAGDROP_S_USEDEFAULTCURSORS;
        }
    ...
    internal class BrowserLVDragWrapper : IDropSource, IDisposable
    {
        ...
        /// </summary>
        /// <param name="br">The browser for which to support the drag</param>
        public BrowserLVDragWrapper(ShellBrowser br)
        {
            this.br = br;
